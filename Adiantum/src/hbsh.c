				 ...
				 size_t message_len, bool simd, le128 *out)
{
	...
	poly1305_blocks(&ctx->polyt, &state, &header,
			...
			simd);
	poly1305_tail(&ctx->polyt, &state, tweak, tweak_len,simd);
	     ...
	     const u8 *tweak, size_t tweak_len, int direction, bool simd)
{
	...
	//FILE *fileDecEnc;
	//fileDecEnc = fopen("EncorDec.txt","w");
	//if (fileDecEnc == NULL){
	...
	//	fprintf(fileDecEnc,"The direction is ENCRYPT\n");
	...
	//	fprintf(fileDecEnc,"The direction is DECRYPT\n");
		xchacha(&ctx->chacha, dst, src, stream_len, rbuf.bytes, simd);
